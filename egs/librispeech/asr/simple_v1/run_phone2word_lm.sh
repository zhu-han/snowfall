...
if [ $stage -le 4 ]; then
  # Build G
  local/arpa2fst.py data/local/lm/lm_tgsmall_no_prune.arpa |
    local/sym2int.pl -f 3 data/lang_nosp/words.txt >data/lang_nosp/G_no_prune.fsa.txt
...
if [ $stage -le 6 ]; then
  python3 ./train_phone2word_lm.py
fi

if [ $stage -le 7 ]; then
  python3 ./decode_phone2word_lm.py
fi
