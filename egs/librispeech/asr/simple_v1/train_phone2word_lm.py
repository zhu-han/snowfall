...
from snowfall.training.ctc_graph import LmCtcTrainingGraphCompiler
...
def get_objf(batch: Dict,
             model: AcousticModel,
             device: torch.device,
             graph_compiler:LmCtcTrainingGraphCompiler,
             training: bool,
...
def main():
    ...
    exp_dir = 'exp-lstm-adam-ctc-phone2wordlm'
    ...
    logging.info("Loading G_no_prune.fsa")
    if (lang_dir / 'G_no_prune.pt').exists():
        G = k2.Fsa.from_dict(torch.load(lang_dir / 'G_no_prune.pt'))
    else:
        with open(lang_dir / 'G_no_prune.fsa.txt') as f:
            G = k2.Fsa.from_openfst(f.read(), acceptor=True)
            G = k2.arc_sort(G)
            torch.save(G.as_dict(), lang_dir / 'G_no_prune.pt')
